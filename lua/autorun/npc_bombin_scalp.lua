...
if SERVER then
    ...
    util.AddNetworkString("BombinScalp_FlareSpawned")

    local SHARED_FLAGS = bit.bor(FCVAR_ARCHIVE, FCVAR_REPLICATED, FCVAR_NOTIFY)

    local cv_enabled     = CreateConVar("npc_bombinscalp_enabled",      "1",    SHARED_FLAGS, "Enable/disable SCALP loitering calls")
    local cv_chance      = CreateConVar("npc_bombinscalp_chance",       "0.12", SHARED_FLAGS, "Probability per check")
    local cv_interval    = CreateConVar("npc_bombinscalp_interval",     "12",   SHARED_FLAGS, "Seconds between NPC checks")
    local cv_cooldown    = CreateConVar("npc_bombinscalp_cooldown",     "50",   SHARED_FLAGS, "Cooldown per NPC")
    local cv_max_dist    = CreateConVar("npc_bombinscalp_max_dist",     "3000", SHARED_FLAGS, "Max call distance")
    local cv_min_dist    = CreateConVar("npc_bombinscalp_min_dist",     "400",  SHARED_FLAGS, "Min call distance")
    local cv_delay       = CreateConVar("npc_bombinscalp_delay",        "5",    SHARED_FLAGS, "Flare to arrival delay")
    local cv_life        = CreateConVar("npc_bombinscalp_lifetime",     "40",   SHARED_FLAGS, "Munition lifetime seconds")
    local cv_speed       = CreateConVar("npc_bombinscalp_speed",        "250",  SHARED_FLAGS, "Forward orbit speed HU/s")
    local cv_radius      = CreateConVar("npc_bombinscalp_radius",       "2500", SHARED_FLAGS, "Orbit radius HU")
    local cv_height      = CreateConVar("npc_bombinscalp_height",       "2500", SHARED_FLAGS, "Altitude above ground HU")
    local cv_dive_damage = CreateConVar("npc_bombinscalp_dive_damage",  "1200", SHARED_FLAGS, "Dive explosion damage")
    local cv_dive_radius = CreateConVar("npc_bombinscalp_dive_radius",  "1200", SHARED_FLAGS, "Dive explosion radius HU")
    local cv_announce    = CreateConVar("npc_bombinscalp_announce",     "0",    SHARED_FLAGS, "Debug prints")
    ...
    local function BS_Debug(msg)
        ...
        local full = "[Bombin SCALP] " .. tostring(msg)
        print(full)
    ...
    local function ThrowSupportFlare(npc, targetPos)
        ...
        net.Start("BombinScalp_FlareSpawned")
    ...
    local function SpawnScalpAtPos(centerPos, callDir)
        if not scripted_ents.GetStored("ent_bombin_scalp") then
            BS_Debug("ent_bombin_scalp not registered")
            return false
        end
        ...
        BS_Debug("SCALP spawned at " .. tostring(centerPos))
        return true
    end

    local function FireScalp(npc, target)
        ...
        timer.Simple(cv_delay:GetFloat(), function()
            ...
            SpawnScalpAtPos(centerPos, storedDir)
        end)

        return true
    end

    timer.Create("BombinScalp_Think", 0.5, 0, function()
        ...
        for _, npc in ipairs(ents.GetAll()) do
            ...
            if not npc.__bombinscalp_hooked then
                npc.__bombinscalp_hooked    = true
                npc.__bombinscalp_nextCheck = now + math.Rand(1, interval)
                npc.__bombinscalp_lastCall  = 0
            end

            if now < npc.__bombinscalp_nextCheck then continue end

            local jitter = math.min(2, interval * 0.5)
            npc.__bombinscalp_nextCheck = now + interval + math.Rand(-jitter, jitter)

            if now - npc.__bombinscalp_lastCall < cv_cooldown:GetFloat() then continue end
            ...
            if FireScalp(npc, enemy) then
                npc.__bombinscalp_lastCall = now
...
if CLIENT then
    ...
    net.Receive("BombinScalp_FlareSpawned", function()
    ...
    hook.Add("Think", "BombinScalp_FlareLight", function()
